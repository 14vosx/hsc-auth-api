...
export async function addCompetitivePlayer(root: string): Promise<void> {
  const totals = { matchesPlayed: 1, mapsPlayed: 1, roundsPlayed: 24, wins: 1, losses: 0, kills: 10, deaths: 8, assists: 3, enemy2ks: 1, enemy3ks: 0, enemy4ks: 0, enemy5ks: 0 };
  ...
  const player = {
    generatedAt: GENERATED_AT, steamid64: STEAM_ID, name: "Fixture",
    lifetime, periods: { "7d": empty, "30d": empty },
    byMap: [{ mapname: "de_dust2", matchesPlayed: 1, mapsPlayed: 1, roundsPlayed: 24, wins: 1, losses: 0, kills: 10, deaths: 8, assists: 3, kdRatio: 99.25, adr: 777.7, impactRating: 42.42 }],
  ...
  await json(path.join(root, "competitive", "players-discovery.json"), {
    counts: { players: 1 }, players: [{ steamid64: STEAM_ID, name: "Fixture", mapsPlayed: 1, matchesPlayed: 1, firstMapAt: "2026-08-13 04:00:00", lastMapAt: "2026-08-13 04:00:00" }],
  });
  await json(path.join(root, "competitive", "players-manifest.json"), {
    generatedAt: GENERATED_AT, counts: { requested: 1, written: 1 },
    players: [{ steamid64: STEAM_ID, name: "Fixture", path: `competitive/player/${STEAM_ID}.json`, summaryMaps: 1, summaryMatches: 1, score: 9876.5 }],
  });
  await json(path.join(root, "competitive", "player", `${STEAM_ID}.json`), player);
  await rewriteChecksums(root);
}
