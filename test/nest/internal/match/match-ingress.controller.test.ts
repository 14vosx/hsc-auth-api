...
test("Match Ingress HTTP Suite", async (t) => {
  ...
  await t.test("503 se matchIngress não estiver configurado", async () => {
  ...
  await t.test("401 se X-HSC-Match-Ingest-Key estiver ausente ou incorreto", async () => {
    await setupApp();
  ...
  await t.test("415 se Content-Type não for application/octet-stream", async () => {
    await setupApp();
    const res = await request(app.getHttpServer())
      .put(`/internal/match/events/${VALID_SOURCE_KEY}/${VALID_EDGE_EVENT_ID}`)
  ...
  await t.test("400 se sourceKey for inválido", async () => {
    await setupApp();
    const res = await request(app.getHttpServer())
  ...
  await t.test("400 se edgeEventId for inválido", async () => {
    await setupApp();
    const res = await request(app.getHttpServer())
  ...
  await t.test("400 se edgeSequence for inválido ou <= 0", async () => {
    await setupApp();
  ...
  await t.test("400 se edgeReceivedAt for timestamp UTC inválido", async () => {
    await setupApp();
    const res = await request(app.getHttpServer())
      .put(`/internal/match/events/${VALID_SOURCE_KEY}/${VALID_EDGE_EVENT_ID}`)
      .set("X-HSC-Match-Ingest-Key", VALID_KEY)
  ...
  await t.test("400 se payloadSha256 header não for 64 hex", async () => {
    await setupApp();
    const res = await request(app.getHttpServer())
      .put(`/internal/match/events/${VALID_SOURCE_KEY}/${VALID_EDGE_EVENT_ID}`)
      .set("X-HSC-Match-Ingest-Key", VALID_KEY)
  ...
  await t.test("400 se recomputação central do SHA-256 divergir do header", async () => {
    await setupApp();
  ...
  await t.test("413 se payload exceder 256 KiB durante streaming", async () => {
    await setupApp();
  ...
  await t.test("400 se UTF-8 for inválido (seqs de bytes malformadas)", async () => {
    await setupApp();
  ...
  await t.test("400 se JSON for malformado", async () => {
    await setupApp();
  ...
  await t.test("400 se JSON não for objeto top-level", async () => {
    await setupApp();
  ...
  await t.test("400 se payload.event for diferente de X-HSC-Event-Name header", async () => {
    await setupApp();
    const res = await request(app.getHttpServer())
      .put(`/internal/match/events/${VALID_SOURCE_KEY}/${VALID_EDGE_EVENT_ID}`)
      .set("X-HSC-Match-Ingest-Key", VALID_KEY)
  ...
  await t.test("series_end: 400 se matchid for ausente/inválido no payload", async () => {
    await setupApp();
  ...
  await t.test("series_end: 400 se X-HSC-Local-Match-Id estiver ausente ou divergente", async () => {
    await setupApp();
  ...
  await t.test("202 e insere no primeiro PUT válido (eventos conhecidos e desconhecidos)", async () => {
    await setupApp();
  ...
  await t.test("Retry idêntico retorna 202 com duplicate=true sem reinserir", async () => {
    await setupApp();
  ...
  await t.test("Retry com payload/conteúdo diferente retorna 409 idempotency_conflict", async () => {
    await setupApp();
  ...
  await t.test("Erro no DB retorna 500 storage_unavailable", async () => {
    const failingRepo = {
      async findBySourceAndEdgeEventId() {
        return null;
      },
